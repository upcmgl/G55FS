...
{
/*
TYPE 	1字节  1-文件系统FLASH 2-升级级字库FLASH 3-铁电 4-文件系统  5-内存
OP   	1字节  0-读取 1-擦除
ADDDR   4字节  地址
LEN		2字节  长度
data  	n字节  下行为参数，上行为数据
*/
    ...
    {
       case 1: //1-文件系统FLASH
    		pageno=bin2_int16u(param+8);//获取待读取页号
            
    		mem_cpy(buffer+pos,param+8,2);//复制 page no
            pos+=2;
...
//			break;
       case 3://3-铁电
          ...
          break;
       case 4://4-文件系统
    	    fileid=bin2_int16u(param+8);//获取待读取文件号
            mem_cpy(buffer+pos,param+8,2);//复制文件ID
            pos+=2;
...
//fread_ertu_params(EEADDR_CLEANNMETER_WARNING,&tmp_updata_verison,16);
    ...
    fread_ertu_params(EEADDR_STAT_FLOW,(INT8U *)&g_app_run_data.gprs_bytes,sizeof(tagAPP_GPRS_BYTES));   //上电首先将nor flash中的流量读出来。
    fread_ertu_params(EEADDR_SET_F702,&g_app_run_data.meter_control.is_control_valid,sizeof(tagAPP_CONTROL));//拉合闸参数
    fread_ertu_params(EEADDR_SET_F701,(INT8U*)&g_app_run_data.password,4);//电表密码
#ifdef __ONENET__
