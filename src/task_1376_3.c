...
uint8_t gprs_set_apn_and_call_net_longsung(void)
{
    ...
    resault = gprs_send_cmd_wait_OK(gprs_cmd_buffer,gprs_cmd_buffer,sizeof(gprs_cmd_buffer),500);
    if(resault >= 0)
    {
        //
        return 1;
...
void  report_erc1_update_info(void)
{
	...
	fread_ertu_params(EEADDR_UPDATA_VERSION,tmp_updata_version,4);
	...
	frame[34]=tmp_updata_version[0];
	frame[35]=tmp_updata_version[1];
	frame[36]=tmp_updata_version[2];
	frame[37]=tmp_updata_version[3];
...
void storeUnsuccessData()
{
	...
	{
		case 1:
			while(!report_warning_to_onenet(1));
			warning_report_cmd =0;
			break;
		case 2:
			while(!report_warning_to_onenet(2));
			warning_report_cmd =0;
			break;
		case 3:
			while(!report_warning_to_onenet(3));
			warning_report_cmd =0;
			break;
		default:
			break;
...
////////////////////////////////////////////////////	 
	//for(int i =0;i<21;i++)
	//{
		//gSystemInfo.managementNum[i]='1';
	//}
	//gSystemInfo.managementNum[21] ='1';
///////////////////////////////////////////////////	 
...
//#ifdef DEBUG
		//if(has_work)
		//{
			//system_debug_info("**********receive some data********");
		//}
//#endif		
        if(gSystemInfo.login_status )
        {
...
#ifdef __EDP__		
		//if(gSystemInfo.update!=0xAA)//不是升级状态
		//{
			//if(minute_elapsed(startTime)>=2)
			//{
				//system_debug_info("\r\n now into edp ping:");
				//edp_ping();
					////if(edp_ping()!=0)    //每隔2分钟要发心跳
					////if(edp_ping()!=0)
					////if(edp_ping()!=0)
				////gprs_power_off();
				//startTime =system_get_tick10ms();
			//}
		//}
		...
		switch(cmdType)   //对从app_protocol_onenet中来的命令数据进行解析
		{
			...
			case 243:   //收到升级命令后首先上报给平台收到升级命令fota 2
				...
				while(!responseUpdataResToOnenet(2));  //收到升级命令
				cmdType =0;
				//DelayNmSec(5000);
				tmp=0xAA;
	...
	//			report645CmdResultToOnenet();
		...
		switch(warning_report_cmd)  //告警上报命令。
		{
			case 1:
				while(!report_warning_to_onenet(1));
				warning_report_cmd =0;
